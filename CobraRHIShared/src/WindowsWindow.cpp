...
namespace GraphicsAbstraction {
	...
	Ref<Window> Window::Create(const WindowProps& props)
	{
		...
		window->impl = new Impl<Window>(props);
		return window;
	}

	Window::~Window()
	{
		RemoveProp(impl->Window, L"Cobra");
	...
	void Window::SetSize(const glm::vec2& size)
	{
		...
		AdjustWindowRectExForDpi(&rect, WS_OVERLAPPEDWINDOW, false, 0, GetDpiForWindow(impl->Window));
		SetWindowPos(impl->Window, HWND_TOP, 0, 0, rect.right - rect.left, rect.bottom - rect.top, SWP_NOACTIVATE | SWP_NOOWNERZORDER | SWP_NOMOVE | SWP_NOZORDER);
	}
	...
	void Window::SetPosition(const glm::vec2& position)
	{
		...
		AdjustWindowRectExForDpi(&rect, WS_OVERLAPPEDWINDOW, false, 0, GetDpiForWindow(impl->Window));
		SetWindowPos(impl->Window, HWND_TOP, rect.left, rect.top, 0, 0, SWP_NOACTIVATE | SWP_NOZORDER | SWP_NOSIZE);
	}

	glm::vec2 Window::GetPosition() const
	{
		POINT pos;
		ClientToScreen(impl->Window, &pos);
	...
	glm::vec2 Window::GetMousePosition() const
	{
		POINT pos;
		GetCursorPos(&pos);
		ScreenToClient(impl->Window, &pos);

		return glm::vec2(pos.x, pos.y);
	}

	bool Window::IsKeyPressed(int keycode) const { return impl->Keys[keycode]; }
	bool Window::IsMouseButtonPressed(int button) const { return impl->MouseButtons[button]; }

	void Window::SetEventCallback(const EventCallbackFn& callback) { impl->Data.EventCallback = callback; }
	void* Window::GetNativeWindow() const { return impl->Window; }
