#include "VulkanResourceHandle.h"
...
namespace GraphicsAbstraction {
	...
	VulkanResourceHandle::VulkanResourceHandle(ResourceType type)
		: m_Type(type)
	{ }

	VulkanResourceHandle::~VulkanResourceHandle()
	{
		if (m_ID.has_value()) s_IDInfos[m_Type].RecycledDescriptors.push_back(m_ID.value());
	}

	uint32_t VulkanResourceHandle::GetValue() const
	{
