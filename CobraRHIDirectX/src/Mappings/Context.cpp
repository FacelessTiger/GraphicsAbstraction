...
namespace GraphicsAbstraction {
	...
	static void debugCallback(D3D12_MESSAGE_CATEGORY category, D3D12_MESSAGE_SEVERITY severity, D3D12_MESSAGE_ID id, LPCSTR pDescription, void* pContext)
	{
		switch (severity)
		{
			case D3D12_MESSAGE_SEVERITY_ERROR: std::cout << pDescription << std::endl; break;
		}
	}
	...
	Ref<Queue> GraphicsContext::GetQueue(QueueType type)
	{
		...
		switch (type)
		{
			case QueueType::Graphics: queue->impl = new Impl<Queue>(reference.GraphicsQueue, D3D12_COMMAND_LIST_TYPE_DIRECT); break;
	...
	ComPtr<IDXGIAdapter4> Impl<GraphicsContext>::SetupAdapter()
	{
	...
	void Impl<GraphicsContext>::SetupDevice(ComPtr<IDXGIAdapter4> adapter)
	{
